...
class CodingEngineTest {
    ...
    fun integerBitfield_encodesWithShift() {
        ...
        transport.put(0x72, 0x3000, byteArrayOf(0x0F, 0, 0, 0, 0, 0, 0, 0))
        ...
        val block = transport.get(0x72, 0x3000)!!
    ...
    fun boolean_preservesOtherBits() {
        val transport = FakeTransport()
        transport.put(0x72, 0x3000, byteArrayOf(0xFE.toByte(), 0, 0, 0, 0, 0, 0, 0))
        val engine = CodingEngine(transport, isDemo = true)
        val item = coding(
            ValueType.BOOLEAN,
            EcuMap(
        ...
        assertEquals(0xFF, transport.get(0x72, 0x3000)!![0].toInt() and 0xFF)
    ...
    fun shortBlock_refusesToGrow() {
        val transport = FakeTransport()
        transport.put(0x72, 0x3000, byteArrayOf(0x00)) // only 1 byte
        val engine = CodingEngine(transport, isDemo = true)
        val item = coding(
            ValueType.INTEGER,
    ...
    fun restoreBlock_writesExactBytesBack() {
        ...
        transport.put(0x72, 0x3000, original)
        val engine = CodingEngine(transport, isDemo = true)
        ...
        assertArrayEquals(original, transport.get(0x72, 0x3000))
    }
